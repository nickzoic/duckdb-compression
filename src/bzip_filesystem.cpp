...
BzipFileHandle::BzipFileHandle(FileSystem &fs, const string &path,
                               FileOpenFlags flags)
...
void BzipFileSystem::MoveFile(const string &source, const string &target,
                              optional_ptr<FileOpener> opener) {
  rename(source.c_str(), target.c_str());
}

bool BzipFileSystem::FileExists(const string &filename,
                                optional_ptr<FileOpener> opener) {
...
vector<OpenFileInfo> BzipFileSystem::Glob(const string &path,
                                          FileOpener *opener) {
  if (FileExists(path))
...
int64_t BzipFileSystem::GetFileSize(FileHandle &handle) { return 0; }

unique_ptr<FileHandle>
BzipFileSystem::OpenFile(const string &path, FileOpenFlags flags,
                         optional_ptr<FileOpener> opener) {
  auto handle = make_uniq<BzipFileHandle>(*this, path, flags);
  return std::move(handle);
}

int64_t BzipFileSystem::Read(FileHandle &handle, void *buffer,
                             int64_t nr_bytes) {
...
int64_t BzipFileSystem::Write(FileHandle &handle, void *buffer,
                              int64_t nr_bytes) {
  auto &fh = handle.Cast<BzipFileHandle>();
